...
SELECT
    ...
    COUNT(
        DISTINCT CASE
            WHEN "value" = 'Healthy'
            AND question_code = '1.3' THEN survey_id
        END
    ) AS "count",
    'Whole grains/cereals are healthy' AS "indicator"
FROM
    {{ ref("unpivot_flatten") }}
GROUP BY
    partner,
...
SELECT
    ...
    COUNT(
        DISTINCT CASE
            WHEN "value" = 'Healthy'
            AND question_code = '1.4' THEN survey_id
        END
    ) AS "count",
    'Whole pulses are healthy' AS "indicator"
FROM
    {{ ref("unpivot_flatten") }}
GROUP BY
    partner,
